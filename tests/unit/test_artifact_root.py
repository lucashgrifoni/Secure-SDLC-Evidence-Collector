...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("the root as a CycloneDX subject", "{root}@1.0", ".@1.0"),
        ("a purl that is not a path", "pkg:pypi/app@1.0", "pkg:pypi/app@1.0"),
...
def test_the_sbom_subject_is_stripped_like_every_other_location(tmp_path: Path) -> None:
    """An SBOM generated by scanning a directory names that directory as its subject.

    `metadata.component.name` is then the absolute path, and it reached
    `bundle.json`, `report.md` and `summary.html` untouched while
    `raw.artifact_path` beside it was stripped correctly — the same flag, the
    same run, one field honouring it and one not. The composition
    `<name>@<version>` is what hid it: the whole string is not a path, so the
    strip declined it.
    """
    repo = (tmp_path / "repo").resolve()
    artifacts = repo / "artifacts"
    artifacts.mkdir(parents=True)
    (artifacts / "sbom.cdx.json").write_text(
        json.dumps(
            {
                "bomFormat": "CycloneDX",
                "specVersion": "1.6",
                "metadata": {
                    "component": {"type": "application", "name": str(repo), "version": "1.0"}
                },
    ...
    sbom = next(e for e in report.evidence if e.evidence_type.value == "sbom")
    assert sbom.subject_ref is not None
    assert str(repo) not in sbom.subject_ref
    assert sbom.subject_ref == ".@1.0"
